...
消退出血は3〜5日ほど続くので、予測を1日の点で出すと実態と合わない。
カレンダーには**期間として**書き、その長さも実測から学習する。

```
長さ = 「生理終わった」まで記録された周期の (終了日 - 開始日 + 1) の中央値
       記録が無ければ既定の4日（3〜5日の中央）
       2日未満・10日超は数え間違いとみなして混ぜない
```

学習するのは「ラグ」と「長さ」の2つになるが、どちらも中央値を取るだけで、
周期長を推定しない点は変わらない。長さは予測の見え方にしか影響せず、
次回がいつ来るかの計算には使わない。
...
| 予測 | `(予測) 🩸 消退出血 ±1日` | 終日・**期間（既定4日）** | 薄いピンク | **先6か月ぶん自動** |
