...
記録・予測・通知はもともと利用者ごとに分かれているので、許可リストに足すだけで
...
2. bot が「まだ登録していません」と返し、**その人の userId を本人に返す**ので、
   ...
   > 運用者側のログからも拾える（`npx wrangler tail` に
   > `unregistered sender: U....` が出る）。記録されるのは userId だけで、
   > メッセージ本文は残さない。

3. 許可リストにカンマ区切りで追加する（既存の値も含めて入力し直す）

   ```bash
   npx wrangler secret put ALLOWED_LINE_USER_ID
   ```

   `U1111...,U2222...` のように入力する。再デプロイは不要。
