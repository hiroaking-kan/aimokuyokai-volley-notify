...
| `追い打ち 2時間` | 飲み忘れ検知をリマインドの何時間後にするか |
| `最終通知 3時間` / `最終通知 なし` | 最終の追い打ち。無効化もできる |
...
| 飲み忘れ検知 | リマインドの2時間後（変更可） | まだ記録がない |
| 飲み忘れ（最終） | リマインドの3時間後（変更可） | まだ記録がない・**実薬日のみ** |
| プラセボ期間の予告 | シート Day 24 の 09:00 | — |
| 消退出血の事前通知 | 予測日の3日前 09:00 | — |
| 出血なしアラート | シート Day 1 の 09:00 | 2シート続けて記録なし |

追い打ちの時刻は**リマインドからの経過時間**で決まる。時刻を固定していると、
リマインドを変えたときに間隔が詰まったり順序が入れ替わったりするため。
