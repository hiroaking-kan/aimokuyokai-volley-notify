...
| 飲み忘れの再通知 | リマインドから10分おき・最大3回（変更可） | まだ記録がない |
| プラセボ期間の予告 | シート Day 24 の 09:00 | — |
| 消退出血の事前通知 | 予測日の3日前 09:00 | — |
| 出血なしアラート | シート Day 1 の 09:00 | 2シート続けて記録なし |

再通知はリマインドからの経過時間で決まり、記録した時点で止まる。
設定を変えると、実際に送られる時刻が返信に出る。

深夜帯（1時〜論理日の切り替わり時刻）に当たる分は送らない。
その場合も返信に「深夜帯のため送信されません」と出るので、黙って落ちることはない。

### 上限がある理由

LINE の無料枠は**アカウント全体で月200通**。記録しないまま放置されると
再通知が枠を食い尽くし、**以降どの通知も届かなくなる**。飲み忘れ防止の
仕組みとして本末転倒なので、回数に上限を持たせている。

```
200通 ÷ 利用者数 ÷ 30日 = 1人1日あたりに使える通数
```

利用者を増やすときや間隔を詰めるときは、この式で見積もり直すこと。

cron は毎分起動している（`* * * * *`）。5分おきだと「7:42」のような指定が
最大5分ずれるため。1日1440回の起動は無料枠に対して誤差。プッシュは28日あたり約40通で、
...
| 利用者 | `allowed_users` テーブル（LINEから追加） | 自分の記録・予測 |

### 定期通知が届く条件

役割ではなく、**実際にピルを飲んでいるか**で決まる。通知が要るのは服薬している
人であって、許可リストに載っている人ではないため。

```
届く = 通知がオン かつ 利用が許可されている

通知がオン: 明示設定があればそれに従う（「通知 オン」「通知 オフ」）
            無ければ自動判定 = シート起点が設定されていれば オン
```

運用のために bot を使うだけの管理者はシートを開始していないので、自動的に
対象外になる。管理者が自分でも飲み始めたら `新しいシート` を送った時点で
自然に通知が始まる。`解除` された人には、設定に関わらず届かない。

**なぜ届くのか / 届かないのかは常に見える。** `状況` と、通知設定を変えた
ときの返信に理由まで出る。届かない通知は気づきにくいので、状態を隠さない。

記録・予測・各種コマンドは管理者でも普通に使える。分かれるのは定期通知だけ。
