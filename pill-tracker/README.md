...
| | 定義 | できること | 定期通知 |
|---|---|---|---|
| 管理者 | シークレット `ALLOWED_LINE_USER_ID`（カンマ区切り） | 利用者の追加・解除 | **届かない** |
| 利用者 | `allowed_users` テーブル（LINEから追加） | 自分の記録・予測 | 届く |

**定期通知（リマインド・飲み忘れ・予測）が届くのは利用者だけ。**
管理者は運用のために bot を使う立場なので、服薬リマインドを送っても邪魔になる。
管理者が自分にも通知が欲しい場合は、`許可 <自分のID>` で利用者としても登録する。

記録・予測・各種コマンドは管理者でも普通に使える。止まるのは定期通知だけ。
